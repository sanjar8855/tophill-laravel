<?php
use App\Models\Product;
?>

@extends('admin.layout')

@section('admin-content')
        <div class="row">
            <div class="col-xl-3 col-sm-6 mb-xl-0 mb-4">
                <div class="card">
                    <div class="card-header p-3 pt-2">
                        <div class="icon icon-lg icon-shape bg-gradient-primary shadow-primary text-center border-radius-xl mt-n4 position-absolute">
                            <i class="fas fa-shopping-cart"></i>
                        </div>
                        <div class="text-end pt-1">
                            <p class="text-sm mb-0 text-capitalize">Kutayotgan buyurtmalar</p>
                            <h4 class="mb-0">{{$waiting}}</h4>
                        </div>
                    </div>
                </div>
            </div>
            <div class="col-xl-3 col-sm-6 mb-xl-0 mb-4">
                <div class="card">
                    <div class="card-header p-3 pt-2">
                        <div class="icon icon-lg icon-shape bg-gradient-success shadow-success text-center border-radius-xl mt-n4 position-absolute">
                            <i class="fas fa-shopping-cart"></i>
                        </div>
                        <div class="text-end pt-1">
                            <p class="text-sm mb-0 text-capitalize">Bajarilgan buyurtmalar</p>
                            <h4 class="mb-0">{{$bajarildi}}</h4>
                        </div>
                    </div>
                </div>
            </div>
            <div class="col-xl-3 col-sm-6 mb-xl-0 mb-4">
                <div class="card">
                    <div class="card-header p-3 pt-2">
                        <div class="icon icon-lg icon-shape bg-gradient-dark shadow-dark text-center border-radius-xl mt-n4 position-absolute">
                            <i class="fas fa-phone-square"></i>
                        </div>
                        <div class="text-end pt-1">
                            <p class="text-sm mb-0 text-capitalize">Kutayotgan bog'lanishlar</p>
                            <h4 class="mb-0">{{$conwait}}</h4>
                        </div>
                    </div>
                </div>
            </div>
            <div class="col-xl-3 col-sm-6">
                <div class="card">
                    <div class="card-header p-3 pt-2">
                        <div class="icon icon-lg icon-shape bg-gradient-info shadow-info text-center border-radius-xl mt-n4 position-absolute">
                            <i class="fas fa-phone-square"></i>
                        </div>
                        <div class="text-end pt-1">
                            <p class="text-sm mb-0 text-capitalize">Bajarilgan bog'lanishlar</p>
                            <h4 class="mb-0">{{$conbaj}}</h4>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <div class="row mb-4 mt-5">
            <div class="col-lg-12 col-md-6 mb-md-0 mb-4">
                <div class="card">
                    <div class="card-header pb-0">
                        <div class="row">
                            <div class="col-lg-6 col-7">
                                <h6>So'ngi 5 ta buyurtma</h6>
                            </div>
                        </div>
                    </div>
                    <div class="card-body px-0 pb-2">
                        <div class="table-responsive p-0">
                            <table class="table align-items-center mb-0">
                                <thead>
                                <tr>
                                    <th class="text-uppercase text-secondary font-weight-bolder opacity-7">T/R</th>
                                    <th class="text-uppercase text-secondary font-weight-bolder opacity-7">Tovar nomi</th>
                                    <th class="text-uppercase text-secondary font-weight-bolder opacity-7">Buyurtmachi ismi</th>
                                    <th class="text-uppercase text-secondary font-weight-bolder opacity-7">Telefon raqami</th>
                                    <th class="text-uppercase text-secondary font-weight-bolder opacity-7">Izoh</th>
                                    <th class="text-uppercase text-secondary text-center font-weight-bolder opacity-7">Status</th>
                                    <th class="text-uppercase text-secondary opacity-7 text-center">Amallar</th>
                                </tr>
                                </thead>
                                <tbody>
                                @foreach($orders as $order)
                                    <tr>
                                        <td>
                                            <p class="font-weight-bold mb-0">{{ ($loop->index +1) }}</p>
                                        </td>
                                        <td>
                                            <a href="{{ route('products.show', ['product' => $order->product_id] )}}" class="mb-0">
                                                {{Product::getNameByProductId($order->product_id)}}
                                            </a>
                                        </td>
                                        <td>
                                            <p class="font-weight-bold mb-0">{{$order->firstname}}</p>
                                        </td>
                                        <td>
                                            <p class="font-weight-bold mb-0">{{$order->phone}}</p>
                                        </td>
                                        <td>
                                            <p class="font-weight-bold mb-0">{{$order->description}}</p>
                                        </td>
                                        <td class="align-middle text-center text-sm">
                                            @if($order->status=='Kutmoqda')
                                                <span class="badge badge-sm bg-gradient-warning">Kutmoqda</span>
                                            @elseif($order->status=='Qabul qilindi')
                                                <span class="badge badge-sm bg-gradient-secondary">Qabul qilindi</span>
                                            @elseif($order->status=='Bajarilmoqda')
                                                <span class="badge badge-sm bg-gradient-info">Bajarilmoqda</span>
                                            @elseif($order->status=='Bajarildi')
                                                <span class="badge badge-sm bg-gradient-success">Bajarildi</span>
                                            @elseif($order->status=='Qaytarildi')
                                                <span class="badge badge-sm bg-gradient-danger">Qaytarildi</span>
                                            @endif
                                        </td>
                                        <td class="align-middle d-flex justify-content-evenly">
                                            <a href="{{ route('orders.show', ['order' => $order->id] )}}" class="btn btn-sm btn-outline-info mb-0" data-toggle="tooltip" data-original-title="Edit user">
                                                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-eye-fill" viewBox="0 0 16 16">
                                                    <path d="M10.5 8a2.5 2.5 0 1 1-5 0 2.5 2.5 0 0 1 5 0z"/>
                                                    <path d="M0 8s3-5.5 8-5.5S16 8 16 8s-3 5.5-8 5.5S0 8 0 8zm8 3.5a3.5 3.5 0 1 0 0-7 3.5 3.5 0 0 0 0 7z"/>
                                                </svg>
                                            </a>
                                            <a href="{{ route('orders.edit', ['order' => $order->id] )}}" class="btn btn-sm btn-outline-warning mb-0" data-toggle="tooltip" data-original-title="Edit user">
                                                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-pencil-fill" viewBox="0 0 16 16">
                                                    <path d="M12.854.146a.5.5 0 0 0-.707 0L10.5 1.793 14.207 5.5l1.647-1.646a.5.5 0 0 0 0-.708l-3-3zm.646 6.061L9.793 2.5 3.293 9H3.5a.5.5 0 0 1 .5.5v.5h.5a.5.5 0 0 1 .5.5v.5h.5a.5.5 0 0 1 .5.5v.5h.5a.5.5 0 0 1 .5.5v.207l6.5-6.5zm-7.468 7.468A.5.5 0 0 1 6 13.5V13h-.5a.5.5 0 0 1-.5-.5V12h-.5a.5.5 0 0 1-.5-.5V11h-.5a.5.5 0 0 1-.5-.5V10h-.5a.499.499 0 0 1-.175-.032l-.179.178a.5.5 0 0 0-.11.168l-2 5a.5.5 0 0 0 .65.65l5-2a.5.5 0 0 0 .168-.11l.178-.178z"/>
                                                </svg>
                                            </a>
                                            <form action="{{route('orders.destroy', ['order' => $order->id])}}" method="post">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="btn btn-sm btn-outline-danger mb-0">
                                                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-trash-fill" viewBox="0 0 16 16">
                                                        <path d="M2.5 1a1 1 0 0 0-1 1v1a1 1 0 0 0 1 1H3v9a2 2 0 0 0 2 2h6a2 2 0 0 0 2-2V4h.5a1 1 0 0 0 1-1V2a1 1 0 0 0-1-1H10a1 1 0 0 0-1-1H7a1 1 0 0 0-1 1H2.5zm3 4a.5.5 0 0 1 .5.5v7a.5.5 0 0 1-1 0v-7a.5.5 0 0 1 .5-.5zM8 5a.5.5 0 0 1 .5.5v7a.5.5 0 0 1-1 0v-7A.5.5 0 0 1 8 5zm3 .5v7a.5.5 0 0 1-1 0v-7a.5.5 0 0 1 1 0z"/>
                                                    </svg>
                                                </button>
                                            </form>
                                        </td>
                                    </tr>
                                @endforeach
                                </tbody>
                            </table>
                        </div>
                    </div>
                </div>
            </div>
        </div>

        <div class="row mb-4 mt-5">
            <div class="col-lg-12 col-md-6 mb-md-0 mb-4">
                <div class="card">
                    <div class="card-header pb-0">
                        <div class="row">
                            <div class="col-lg-6 col-7">
                                <h6>So'ngi 5 ta bog'lanishlar</h6>
                            </div>
                        </div>
                    </div>
                    <div class="card-body px-0 pb-2">
                        <div class="table-responsive p-0">
                            <table class="table align-items-center mb-0">
                                <thead>
                                <tr>
                                    <th class="text-uppercase text-secondary font-weight-bolder opacity-7">T/R</th>
                                    <th class="text-uppercase text-secondary font-weight-bolder opacity-7">Nomi</th>
                                    <th class="text-uppercase text-secondary font-weight-bolder opacity-7">Telefon</th>
                                    <th class="text-uppercase text-secondary font-weight-bolder opacity-7">Xat</th>
                                    <th class="text-uppercase text-secondary font-weight-bolder opacity-7">Izoh</th>
                                    <th class="text-uppercase text-secondary text-center font-weight-bolder opacity-7">Status</th>
                                    <th class="text-uppercase text-secondary opacity-7 text-center">Amallar</th>
                                </tr>
                                </thead>
                                <tbody>
                                @foreach($contacts as $contact)
                                    <tr>
                                        <td>
                                            <p class="font-weight-bold mb-0">{{ ($loop->index +1) }}</p>
                                        </td>
                                        <td>
                                            <p class="font-weight-bold mb-0">{{$contact->name}}</p>
                                        </td>
                                        <td>
                                            <p class="font-weight-bold mb-0">{{$contact->phone}}</p>
                                        </td>
                                        <td>
                                            <p class="font-weight-bold mb-0">{{$contact->message}}</p>
                                        </td>
                                        <td>
                                            <p class="font-weight-bold mb-0">{{$contact->description}}</p>
                                        </td>
                                        <td class="align-middle text-center text-sm">
                                            @if($contact->status=='Kutmoqda')
                                                <span class="badge badge-sm bg-gradient-warning">Kutmoqda</span>
                                            @elseif($contact->status=='Bajarildi')
                                                <span class="badge badge-sm bg-gradient-success">Bajarildi</span>
                                            @elseif($contact->status=='Bajarilmadi')
                                                <span class="badge badge-sm bg-gradient-danger">Bajarilmadi</span>
                                            @endif
                                        </td>
                                        <td class="align-middle d-flex justify-content-evenly">
                                            <a href="{{ route('contacts.show', ['contact' => $contact->id] )}}" class="btn btn-sm btn-outline-info mb-0" data-toggle="tooltip" data-original-title="Edit user">
                                                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-eye-fill" viewBox="0 0 16 16">
                                                    <path d="M10.5 8a2.5 2.5 0 1 1-5 0 2.5 2.5 0 0 1 5 0z"/>
                                                    <path d="M0 8s3-5.5 8-5.5S16 8 16 8s-3 5.5-8 5.5S0 8 0 8zm8 3.5a3.5 3.5 0 1 0 0-7 3.5 3.5 0 0 0 0 7z"/>
                                                </svg>
                                            </a>
                                            <a href="{{ route('contacts.edit', ['contact' => $contact->id] )}}" class="btn btn-sm btn-outline-warning mb-0" data-toggle="tooltip" data-original-title="Edit user">
                                                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-pencil-fill" viewBox="0 0 16 16">
                                                    <path d="M12.854.146a.5.5 0 0 0-.707 0L10.5 1.793 14.207 5.5l1.647-1.646a.5.5 0 0 0 0-.708l-3-3zm.646 6.061L9.793 2.5 3.293 9H3.5a.5.5 0 0 1 .5.5v.5h.5a.5.5 0 0 1 .5.5v.5h.5a.5.5 0 0 1 .5.5v.5h.5a.5.5 0 0 1 .5.5v.207l6.5-6.5zm-7.468 7.468A.5.5 0 0 1 6 13.5V13h-.5a.5.5 0 0 1-.5-.5V12h-.5a.5.5 0 0 1-.5-.5V11h-.5a.5.5 0 0 1-.5-.5V10h-.5a.499.499 0 0 1-.175-.032l-.179.178a.5.5 0 0 0-.11.168l-2 5a.5.5 0 0 0 .65.65l5-2a.5.5 0 0 0 .168-.11l.178-.178z"/>
                                                </svg>
                                            </a>
                                            <form action="{{route('contacts.destroy', ['contact' => $contact->id])}}" method="post">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="btn btn-sm btn-outline-danger mb-0">
                                                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-trash-fill" viewBox="0 0 16 16">
                                                        <path d="M2.5 1a1 1 0 0 0-1 1v1a1 1 0 0 0 1 1H3v9a2 2 0 0 0 2 2h6a2 2 0 0 0 2-2V4h.5a1 1 0 0 0 1-1V2a1 1 0 0 0-1-1H10a1 1 0 0 0-1-1H7a1 1 0 0 0-1 1H2.5zm3 4a.5.5 0 0 1 .5.5v7a.5.5 0 0 1-1 0v-7a.5.5 0 0 1 .5-.5zM8 5a.5.5 0 0 1 .5.5v7a.5.5 0 0 1-1 0v-7A.5.5 0 0 1 8 5zm3 .5v7a.5.5 0 0 1-1 0v-7a.5.5 0 0 1 1 0z"/>
                                                    </svg>
                                                </button>
                                            </form>
                                        </td>
                                    </tr>
                                @endforeach
                                </tbody>
                            </table>
                        </div>
                    </div>
                </div>
            </div>
        </div>



@endsection()
